...
async function fetchM3U() {
    ...
    channels = parsed.items
        .filter(item => item.url && item.url.includes('.m3u8')) // Only .m3u8 for better buffering
        .map((item, index) => {
...
async function buildAddon() {
    await fetchM3U(); // preload channels, lazy EPG
